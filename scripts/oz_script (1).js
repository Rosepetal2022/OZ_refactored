...
function Gallery_tour(images_path, gallery_path) {
  ...
  this.build_gallery = function(data) {
    ...
    self.gallery.children('div.holder').children().eq(self.starting_point).bind('reloaded', self.make_gallery);
    self.gallery.children('div.holder').children().eq(self.starting_point).trigger('reloaded', self.make_gallery);
  };
  ...
  this.make_gallery = function(e) {
    if (!e.currentTarget.complete) { $(e.currentTarget).bind('load', self.make_gallery); return false;}
...
function Article_reader() {
  ...
  this.bring_article = function(e) {
    ...
    self.sketcher.draw(self.article_basin, {opacity : 0 }, function() {
      if (!self.large_article[0].complete && self.large_article[0].complete != undefined) {
        window_in();
//        self.large_article.bind('load', window_in); //I don't understand this DMR
      } else {
        window_in();
      }
    });
